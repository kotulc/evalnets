...
def main(_):
    ...
    with tf.name_scope('conv'):
        ...
        # Reshape and apply dropout to conv output
        h_flat = tf.reshape(h_conv, [-1, 13 * 13 * FLAGS.conv_fmaps])
        h_out_drop = tf.nn.dropout(h_flat, keep_prob)


    # Readout layer
    with tf.name_scope('readout'):
        W_out = weight_variable([13 * 13 * FLAGS.conv_fmaps, num_classes])
        b_out = bias_variable([num_classes])

        y = tf.nn.softmax(tf.matmul(h_out_drop, W_out) + b_out)
